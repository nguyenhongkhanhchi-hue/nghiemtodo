          ...
          <>
            ...
            <div className="bg-[var(--bg-elevated)] rounded-2xl p-4 border border-[var(--border-subtle)]">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <p className="text-xs font-bold text-[var(--text-primary)]">Đơn giá 1 giờ làm việc</p>
                  <p className="text-[10px] text-[var(--text-muted)]">Dùng để tính Chi phí thời gian</p>
                </div>
                {editingRate ? (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-28 bg-[var(--bg-surface)] rounded-lg px-2 py-1 text-sm font-mono text-right text-[var(--text-primary)] border border-[var(--border-accent)] outline-none"
                      autoFocus
                    />
                    <button
                      onClick={() => {
                        const v = parseInt(rateInput);
                        if (v > 0) { setHourlyRate(v); localStorage.setItem('nw_hourly_rate', v.toString()); }
                        setEditingRate(false);
                      }}
                      className="px-2 py-1 rounded-lg bg-[var(--accent-primary)] text-white text-xs font-bold"
                    >Lưu</button>
                  </div>
                ) : (
                  <button
            ...
            <div className="bg-[var(--bg-elevated)] rounded-2xl p-4 border border-[var(--border-accent)] relative overflow-hidden">
              ...
              <h3 className="text-xs font-bold text-[var(--text-secondary)] mb-3 flex items-center gap-1.5">
                <span className="size-5 rounded-full bg-[var(--accent-dim)] flex items-center justify-center text-[var(--accent-primary)] text-[10px]">⏱</span>
                Hiệu suất hôm nay
              </h3>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[var(--bg-surface)] rounded-xl p-3">
                  <p className="text-[10px] text-[var(--text-muted)] mb-1">Thời gian theo dõi</p>
                  <p className="text-base font-black font-mono text-[var(--accent-primary)]">
                  ...
                  <div className="mt-1.5 h-1.5 rounded-full bg-[var(--bg-elevated)] overflow-hidden">
                    <div className="h-full rounded-full bg-[var(--accent-primary)]" style={{ width: `${todayStats.timeEfficiency}%` }} />
                  </div>
                  <p className="text-[9px] text-[var(--text-muted)] mt-0.5">{todayStats.timeEfficiency.toFixed(0)}% của 24h</p>
                </div>
                <div className="bg-[var(--bg-surface)] rounded-xl p-3">
                  <p className="text-[10px] text-[var(--text-muted)] mb-1">Thời gian phung phí</p>
                  <p className="text-base font-black font-mono text-[var(--error)]">
                    {Math.floor(todayStats.wastedHours)}h{String(Math.round((todayStats.wastedHours % 1) * 60)).padStart(2, '0')}m
                  </p>
                  <p className="text-[9px] text-[var(--text-muted)] mt-1">{(100 - todayStats.timeEfficiency).toFixed(0)}% chưa theo dõi</p>
                </div>
                <div className="bg-[var(--bg-surface)] rounded-xl p-3">
                  <p className="text-[10px] text-[var(--text-muted)] mb-1">Chi phí thời gian</p>
                  <p className="text-sm font-black font-mono text-[var(--warning)]">{formatMoney(todayStats.trackedTimeCost)}đ</p>
                  <p className="text-[9px] text-[var(--text-muted)] mt-0.5">Chi phí 24h: {formatMoney(todayStats.cost24h)}đ</p>
                </div>
                <div className="bg-[var(--bg-surface)] rounded-xl p-3">
                  <p className="text-[10px] text-[var(--text-muted)] mb-1">Dòng tiền hôm nay</p>
                  <p className={`text-sm font-black font-mono ${todayStats.dayFlow >= 0 ? 'text-[var(--success)]' : 'text-[var(--error)]'}`}>
                    {todayStats.dayFlow >= 0 ? '+' : ''}{formatMoney(todayStats.dayFlow)}đ
                  </p>
                  <p className={`text-[9px] mt-0.5 ${todayStats.dayFlow >= 0 ? 'text-[var(--success)]' : 'text-[var(--error)]'}`}>
                    {todayStats.dayFlow >= 0 ? '✓ Ngày có lãi' : '✗ Ngày thua lỗ'}
                  </p>
                </div>
              </div>
              {/* Income / Expense row */}
              <div className="grid grid-cols-2 gap-3 mt-3">
                <div className="bg-[var(--bg-surface)] rounded-xl p-3 flex items-center gap-2">
                  <TrendingUp size={14} className="text-[var(--success)] flex-shrink-0" />
                  <div>
                    <p className="text-[10px] text-[var(--text-muted)]">Tổng thu hôm nay</p>
                    <p className="text-sm font-bold font-mono text-[var(--success)]">{formatMoney(todayStats.totalIncome)}đ</p>
                  </div>
                </div>
                <div className="bg-[var(--bg-surface)] rounded-xl p-3 flex items-center gap-2">
                  <TrendingDown size={14} className="text-[var(--error)] flex-shrink-0" />
                  <div>
                    <p className="text-[10px] text-[var(--text-muted)]">Tổng chi hôm nay</p>
                    <p className="text-sm font-bold font-mono text-[var(--error)]">{formatMoney(todayStats.totalExpense)}đ</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border-subtle)] overflow-hidden">
              <div className="px-4 py-3 border-b border-[var(--border-subtle)]">
                <h3 className="text-xs font-bold text-[var(--text-secondary)]">Dòng tiền từng việc</h3>
                <p className="text-[10px] text-[var(--text-muted)] mt-0.5">{cashFlowData.length} việc đã hoàn thành</p>
              </div>
                  ...
                  {cashFlowData.map(item => (
                    <div key={item.id} className="px-4 py-3">
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        <p className="text-xs font-medium text-[var(--text-primary)] flex-1 leading-tight">{item.title}</p>
                        <span className={`flex-shrink-0 text-[10px] font-bold px-1.5 py-0.5 rounded-md ${item.isProfit ? 'bg-green-500/15 text-green-400' : 'bg-red-500/15 text-red-400'}`}>
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        {item.durationSec > 0 && (
                          <span className="text-[10px] text-[var(--text-muted)]">
                            ⏱ {Math.floor(item.durationSec / 3600)}h{String(Math.floor((item.durationSec % 3600) / 60)).padStart(2, '0')}m
                            <span className="text-[var(--warning)] ml-1">(-{formatMoney(item.timeCost)}đ)</span>
                          </span>
                        )}
                        {item.income > 0 && (
                          <span className="text-[10px] text-[var(--success)]">+{formatMoney(item.income)}đ Thu</span>
                        )}
                        {item.expense > 0 && (
                          <span className="text-[10px] text-[var(--error)]">-{formatMoney(item.expense)}đ Chi</span>
                        )}
                        <span className={`text-[10px] font-bold ml-auto ${item.netFlow >= 0 ? 'text-[var(--success)]' : 'text-[var(--error)]'}`}>
